...
function confirmarEliminar(id: string, nombre: string) {
  if (confirm(`Eliminar el vehiculo ${nombre}?`)) {
    store.eliminar(id)
  }
}
</script>
  ...
  <div class="min-h-screen bg-bg dark:bg-dk-bg transition-colors duration-300">
    ...
    >
      ...
      <main class="flex-1 px-12 py-10 max-[768px]:px-5 max-[768px]:py-6">
        ...
        <!-- Grid -->
        <div class="grid grid-cols-[repeat(auto-fill,minmax(30rem,1fr))] gap-6">
          <div
          ...
          >
            ...
            <div class="flex items-center gap-4 mb-4 text-[1.2rem] text-muted dark:text-dk-muted">
              <span>{{ v.marca }}</span>
            ...
            >
              ...
              <div class="flex items-center gap-2" @click.stop>
                <button
                  ...
                  title="Editar"
                  @click="router.push({ name: 'vehiculos-editar', params: { id: v.id } })"
                >
                  <svg
                    width="14"
                    height="14"
        ...
        <div v-if="vehiculosFiltrados.length === 0" class="py-16 text-center">
