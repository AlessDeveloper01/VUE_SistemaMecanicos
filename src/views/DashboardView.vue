...
const metrics = computed(() => [
  ...
  {
    value: String(
      ordStore.ordenes.filter((o) => o.estado !== 'completado' && o.estado !== 'cancelado').length,
    ),
...
const mapEstado: Record<EstadoOrden, 'proceso' | 'pendiente' | 'completado'> = {
  pendiente: 'pendiente',
  en_proceso: 'proceso',
...
const recordatorios = computed(() =>
  ordStore.ordenes
    ...
    .slice(0, 3)
    .map((o) => ({
      title: o.vehiculoNombre,
      description: `${o.clienteNombre} — ${o.diagnostico || 'Servicio pendiente'}`,
  ...
  <div class="min-h-screen bg-bg dark:bg-dk-bg transition-colors duration-300">
    ...
    >
      ...
      <main class="flex-1 px-12 py-10 max-[768px]:px-5 max-[768px]:py-6">
        ...
        <!-- Main layout -->
        <div class="grid grid-cols-[1fr_36rem] gap-14 max-[1200px]:grid-cols-1">
          <!-- Left -->
          <div>
            <!-- Vehiculos en taller -->
            <section class="mb-14">
              ...
              />
              <div v-if="vehiculosEnTaller.length === 0" class="py-8 text-center">
            ...
            <div class="grid grid-cols-2 gap-12 max-[768px]:grid-cols-1">
              <!-- Ordenes -->
              <section>
                ...
                <OrderItem
                  ...
                  :id="o.id"
                  :title="o.vehiculoNombre"
                  :subtitle="o.diagnostico || o.servicios.map((s) => s.descripcion).join(', ')"
                  :mechanic="o.mecanico || 'Sin asignar'"
                  :time="o.fechaIngreso"
                  :status="mapEstado[o.estado]"
                />
                <div v-if="ultimasOrdenes.length === 0" class="py-8 text-center">
                  <p class="text-[1.3rem] text-muted dark:text-dk-muted">
              ...
              <section>
                ...
                <ReminderItem
                  ...
                  :due-date="r.dueDate"
                />
                <div v-if="recordatorios.length === 0" class="py-8 text-center">
                  <p class="text-[1.3rem] text-muted dark:text-dk-muted">Todo al dia</p>
                </div>
              </section>
            </div>
          </div>
          ...
          <div>
            ...
            <section class="mb-14">
              ...
              <PartItem
                ...
                :status="p.status"
              />
              <div v-if="itemsBajos.length === 0" class="py-8 text-center">
                <p class="text-[1.3rem] text-muted dark:text-dk-muted">Stock en niveles normales</p>
              </div>
            </section>
